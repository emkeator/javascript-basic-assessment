...
var hello = 'goodbye'; //you say goobye, and I say hello...
...
myOtherShirt['type'] = 'spandex'; //ew why spandex
...
for (let parseltongue in snake) {
	if (parseltongue[0] === 's') {
		snake[parseltongue] = 's';
	}
}
...
//^ew no spongebob is NOT in the same class as wonder woman
...
function removeItem(aryaStarksKillList, victim) {
	while (aryaStarksKillList.indexOf(victim) !== -1) {
		aryaStarksKillList.splice(aryaStarksKillList.indexOf(victim), 1);
	}
	return aryaStarksKillList;
}

// #11 Write a function  called doubleTheFun that takes 1 parameter. It should double numbers, and
// repeats strings. example 4->8, 2.5->5, 'Awesome'->'AwesomeAwesome'

var doubleTheFun = fredAndGeorge => (typeof fredAndGeorge !== 'string' || Number(fredAndGeorge)) ? fredAndGeorge * 2 : fredAndGeorge += fredAndGeorge;


// #12 Write function getValueOfProperty that takes in an object, and the name of a property on the object
// return the value from the object that corresponds to the property

function getValueOfProperty(horcrux, property) {
    return horcrux[property];
}
...
//Jeremy and then changes the person object to have a property called lovesCode with a value of 10.  If their name is Brack 
...
*/
const outside = (temp, humidity, cloudiness) => {
